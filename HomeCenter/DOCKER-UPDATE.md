# Обновление приложения в контейнере HomeCenter
...
- **Контейнер:** `docker-compose.yml` — сервис `homecenter`, порт 8080, образ `homecenter`.
- **Volumes:** `./tests` и `./Schoolbook` примонтированы в контейнер — изменения в файлах тестов и учебников на хосте сразу видны в контейнере, пересборка не нужна.
...
Выполнять в каталоге с `docker-compose.yml` (корень HomeCenter). Можно использовать скрипт `update-container.ps1` (см. конец раздела).
...
- **Только изменились файлы в папке `Schoolbook/`** — папка смонтирована как volume (`./Schoolbook:/app/Schoolbook`). Достаточно обновить страницу «Учебники».
...
| Ситуация                                     | Действие                                                       |
| -------------------------------------------- | -------------------------------------------------------------- |
| Поменялся код (C#, Views, Dockerfile и т.д.) | `docker compose up -d --build` в папке HomeCenter                 |
| Поменялись только файлы в `tests/` или `Schoolbook/` | Ничего не делать с контейнером                                 |
| Нужна полная пересборка без кэша             | `docker compose build --no-cache` затем `docker compose up -d`  |

### Скрипт update-container.ps1

В корне HomeCenter можно запустить:
...
2. **Сборка с сетевого/синхронизируемого каталога**  
   Если проект на сетевом диске, в OneDrive/Google Drive или в папке с ограничениями, сборка может падать. **Скопируйте проект на локальный диск** (например `C:\Projects\HomeCenter`) и запускайте `docker compose up -d --build` оттуда.
