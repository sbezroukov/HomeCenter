...
Каждый блок задаёт **один файл** теста. В начале блока указывается путь к файлу, затем — содержимое в формате HomeCenter (см. `tests/README.md`).
...
Сразу после строки `ФАЙЛ:` идёт содержимое файла в формате HomeCenter:
...
Q: Какие города являются столицами? (выберите все верные)
1) Санкт-Петербург
*2) Москва
*3) Берлин
4) Нижний Новгород
...
6. **Test:** варианты с `*` у правильного. Один `*` — выбор одного ответа; несколько `*` — выбор нескольких правильных (чекбоксы). **Open:** при необходимости — секция `---`, строка `Ответы:` или `ОТВЕТЫ:`, затем нумерация (`1.`, `2.` и т.д.).
7. **Кодировка:** UTF-8.
