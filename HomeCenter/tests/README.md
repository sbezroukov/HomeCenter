# Формат файлов тестов HomeCenter
...
|---------------|------------|-------------------|
| **Test**      | Тест с вариантами ответа | Варианты с `*` — правильные. Один вариант = один ответ (радио); несколько вариантов с `*` = выбор нескольких (чекбоксы). Результат в процентах. |
...
- **Один правильный вариант** — пользователь выбирает один ответ (radio). **Несколько правильных вариантов** — пользователь отмечает все нужные (checkboxes); ответ засчитывается только при полном совпадении (все правильные выбраны, ни одного лишнего).
- Варианты заканчиваются при первой пустой строке или при следующей строке `Q:`.

Пример (один правильный):
...
Пример (несколько правильных):

```text
MODE: Test

Q: Какие из перечисленных городов являются столицами?
1) Санкт-Петербург
*2) Москва
3) Нижний Новгород
*4) Берлин
...
5. **Test:** у каждого вопроса — несколько строк вариантов. Один или несколько вариантов могут начинаться с `*` (правильные). Один `*` → выбор одного ответа (radio); несколько `*` → выбор нескольких (checkboxes). Варианты заканчиваются пустой строкой или следующим `Q:`.
